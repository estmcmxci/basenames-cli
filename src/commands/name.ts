  ...
  try {
    ...
    if (checkCompatibility) {
      ...
      if (compat.hasOwner) {
        ...
        console.log(
          colors.dim("  This is informational only - naming will proceed.")
        );
      }
    }
    ...
    if (!isOwner) {
      ...
      console.error(
        colors.yellow("Please ensure you're using the correct private key")
      );
    ...
    if (!noReverse) {
      ...
      try {
        ...
        console.log(
          colors.yellow(`⚠ Could not set reverse resolution: ${e.message}`)
        );
        console.log(
          colors.dim("  This is non-critical. Forward resolution was successful.")
        );
      }
    } else {
    ...
    stopSpinner();
    const e = error as Error;
    console.error(colors.red(`Error naming contract: ${e.message}`));
  } finally {
    if (useLedger) {
      await closeLedger();
    }
